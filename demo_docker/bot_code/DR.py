...
from bot_code.zimply import ZIMFile
...
class DocumentRetriever(object):
    def __init__(self, filename, indexdir="../index_small", encoding="utf-8", limitmb=5000, procs=8, mlimit=4000000000, pages=None):
        self._zim_file = ZIMFile(filename, encoding) # ZIM
        ...
        if not os.path.exists(indexdir):
            ...
            articleCount = self._zim_file.header_fields['articleCount'] # ZIM
            #articleCount = 1
    ...
    def get_data_by_idx(self, idx):
        entry = self._zim_file.read_directory_entry_by_index(idx) # ZIM
        #entry = 'ZIM'
        if entry['namespace'] == "A":
            try:
                return self._zim_file._read_blob(entry['clusterNumber'], entry['blobNumber']).decode(self.encoding) # ZIM
                #return 0 # ZIM
            except:
                return None
        else:
...
def main():
    with DocumentRetriever(sys.argv[1], sys.argv[2]) as dr:
        start = time.time()
